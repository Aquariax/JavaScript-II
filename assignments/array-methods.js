...
runners.forEach(function(runners){
    const dell = (runners.first_name +" "+ runners.last_name);
    fullName.push(dell) ;
})
...
let ticketPriceTotal = runners.reduce(function(amount, runners){
    return amount + runners.donation;
}, 0);
...
// Problem 1
const payless = runners.filter(function(runners){
    return runners.shirt_size === "S";
});
console.log(payless)
// Problem 2
const moneyWise = runners.map(function(payless){
    return payless.last_name.toLowerCase();
})
console.log(moneyWise)
// Problem 3
let sizedUp = runners.filter(function(payless){
    return payless.donation >= 100;
}, 0);
console.log(sizedUp);